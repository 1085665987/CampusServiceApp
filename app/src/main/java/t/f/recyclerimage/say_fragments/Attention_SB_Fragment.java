...
public class Attention_SB_Fragment extends Fragment implements OnRefreshListener, OnLoadMoreListener ,ForumAdapter.OnItemClickListener{
    public static String TAG="Attention_SB_Fragment";
    ...
    public void onLoadMore() {
        ...
        String url=getActivity().getResources().getString(R.string.dongtai_attention)+ "?num="+count+"&phone="+userInfo.getLongInfo(Property.NO);
        Log.e(TAG,url);
    ...
    private void getFriendCircleData(){
        UserInfo userInfo=new UserInfo(getActivity());
        String url=getActivity().getResources().getString(R.string.dongtai_attention)+ "?num="+0+"&phone="+userInfo.getLongInfo(Property.NO);
        Log.e(TAG,url);
        OkHttpUtils.get().url(url).build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
    ...
    /* 先实例化dialog并进行相关设置 */
        ...
        LinearLayout ll = (LinearLayout) getLayoutInflater().inflate(R.layout.forum_dialog, null);
        ...
        //关注此人，先判断是不是本人的帖子，如果是 不显示本行，不是的话，就关注此人
        if(forumBean.getIsMine()) {
            ll.findViewById(R.id.forum_line_2).setVisibility(View.GONE);
            LinearLayout interest = (LinearLayout) ll.findViewById(R.id.interest);
            interest.setVisibility(View.GONE);
            interest.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast.makeText(getActivity(), "关注此人", Toast.LENGTH_SHORT).show();
...
//                forumBean.setCommentCount(mJsonObject.getInt(Property.forum_comment_count_key));       //设置帖子的评论人数
                forumBean.setCommentCount(3);       //设置帖子的评论人数
