...
import okhttp3.Call;
import t.f.recyclerimage.Forum_Details_Activity;
...
public class AllSchool_Fragment extends Fragment implements OnRefreshListener, OnLoadMoreListener ,ForumAdapter.OnItemClickListener{
    public static String TAG="AllSchool_Fragment";
    ...
    public void onLoadMore() {
        ...
        String url=getActivity().getResources().getString(R.string.dongtai_down)+ "?num="+count+"&phone="+userInfo.getLongInfo(Property.NO);
        Log.e(TAG,url);
    ...
    public void onRefresh() {
        ...
        OkHttpUtils.get().url(url).build().execute(new StringCallback() {
            ...
            public void onResponse(String response, int id) {
                addData(response);
                forumAdapter.notifyItemRangeInserted(0,forumList.size());
                swipeToLoadLayout.setRefreshing(false);
            }
        });
    }
    ...
    private void initView(){
        ...
        swipeToLoadLayout.setRefreshCompleteToDefaultScrollingDuration(1000);
...
//        ((DefaultItemAnimator) mRecycleView.getItemAnimator()).setSupportsChangeAnimations(false);
        ...
        animation.setChangeDuration(1000);
        ...
        mRecycleView.getItemAnimator().setMoveDuration(1000);
    ...
    private void getFriendCircleData(){
        UserInfo userInfo=new UserInfo(getActivity());
        String url=getActivity().getResources().getString(R.string.dongtai_down)+ "?num="+0+"&phone="+userInfo.getLongInfo(Property.NO);
        Log.e(TAG,url);
        OkHttpUtils.get().url(url).build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
    ...
    /* 先实例化dialog并进行相关设置 */
        ...
        if(forumBean.getIsMine()) {
            ...
            String url=getActivity().getResources().getString(R.string.dongtai_is_attention)+"?dongtai_id="+forumBean.getForumId()+"&phone="+phone;
            Log.i(TAG,url);
        ...
        try {
            ...
            for(int i=0;i<jsonArray.length();i++){
                ...
                for(int j=0;j<pictureJsons.length();j++){
                    Log.i(TAG+"addData()",pictureJsons.getString(j));
    ...
    private void doLove(int position){
        ...
        if(!forumBean.isLoved()) {
            ...
            mRecycleView.getItemAnimator().setChangeDuration(0);
            forumAdapter.notifyItemRangeChanged(position,1);
        }else{
    ...
    private void doCheakForum(int position){
        Toast.makeText(getActivity(),"点击了第"+position+"个帖子",Toast.LENGTH_SHORT).show();

        ForumBean forumBean=forumList.get(position);

        Intent intent=new Intent(getActivity(), Forum_Details_Activity.class);
        intent.putExtra(Property.FORUM_DETAILS_AVTIVITY_PARAM_KEY, forumBean);
